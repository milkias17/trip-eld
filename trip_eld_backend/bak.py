...
class Transformer:
    ...
    def transform(self):
        for seg_idx, seg in enumerate(self.route.get("segments", [])):
            ...
            for step_idx, step in enumerate(seg.get("steps", [])):
                ...
                remaining_time = float(step_duration)
                remaining_distance = float(step_distance)
                ...
                while remaining_time > eps and remaining_distance > eps:
                    to_break, to_rest, to_cycle = self.seconds_to_thresholds()
                    dist_to_fuel = max(0.0, DISTANCE_LIMIT - self.cumulative_distance)
                    if dist_to_fuel > 0:
                        time_to_fuel = predict_duration(
                            remaining_time, remaining_distance, dist_to_fuel
                        )
                    elif dist_to_fuel <= 0:
                        time_to_fuel = 0.0
                    else:
                        time_to_fuel = float("inf")
                    ...
                    chunk_distance = predict_distance(
                        remaining_time, remaining_distance, next_cut
                    )
                    chunk_distance = max(0.0, min(chunk_distance, remaining_distance))

                    self.record_drive(
                        next_cut, chunk_distance, seg_idx, step_idx, instruction
                    )

                    remaining_time -= next_cut
                    remaining_distance -= chunk_distance
                    distance_covered_in_step += chunk_distance
    ...
    def record_drive(
        ...
        self.cumulative_driving += float(duration_seconds)
        self.consecutive_driving += float(duration_seconds)
        self.cumulative_on_duty += float(duration_seconds)
        self.seconds_elapsed += duration_seconds
        self.cumulative_distance += event["distance_meters"]
        self.total_cycle_on_duty += float(duration_seconds)
